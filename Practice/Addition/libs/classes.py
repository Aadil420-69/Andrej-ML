...
def get_batch(split: str, config: 'GPTConfig', iterr: int = -10):
	...
	iterr = -10 specifies that get_batch is called from the training loop
	not while estimating the loss
	...
	# print(data.shape)
	ix = torch.randint(len(data) * config.data.res_number, (config.batch_size, ))
	# print(f'{ix.shape=}')

	# print(f'{[config.data.encode(data[i//config.data.res_number], i % config.data.res_number) for i in ix]=}')
	t = torch.stack([config.data.encode(data[i//config.data.res_number], i % config.data.res_number, True) for i in ix])
	x = t[:, 0, :].to(device=config.device)
	y = t[:, 1, :].to(device=config.device)

	with open('Practice/GPT/Logs/log.txt', 'w') as f:
...
class Block(nn.Module):
	...
	def forward(self, x: torch.Tensor):
		...
		"""
		x = x + self.sa(self.ln1(x))    # RNN
		# print('Block-1', tuple(x.shape))
		x = x + self.ffwd(self.ln2(x))    # RNN
		# print('Block-2', tuple(x.shape))
		return x
...
class GPT(nn.Module):
	...
	def __init__(self, config: 'GPTConfig' = None):
		...
		if config:
			...
			self.token_embedding_table = nn.Embedding(config.data.vocab_size, config.n_embd)
			self.position_embedding_table = nn.Embedding(config.data.block_size, config.n_embd)
			self.blocks = nn.Sequential(*[Block(config) for _ in range(config.n_layer)])
			self.ln_f = nn.LayerNorm(config.n_embd)	# final layer norm
			self.lm_head = nn.Linear(config.n_embd, config.data.vocab_size)
	...
	def generate_max(self, idx, max_new_tokens):
		"""_summary_
		...
		for _ in range(max_new_tokens):
			...
			idx_cond = idx[:, -config.data.block_size:]
			# get the Predicitons
	...
	def generate_mul(self, idx, max_new_tokens):
		"""_summary_
...
# hyperparameters
class GPTConfig:
	def __init__(self, data: 'DataClass', max_number: int,
			  batch_size=64, max_iters: int = 5000, eval_intervals: int = 500,
			  learning_rate: float = 3e-4, eval_iters: int = 200,
			  n_embd: int = 384, n_head: int = 6, n_layer: int = 6,
