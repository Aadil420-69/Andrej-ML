...
def get_batch(split: str, config: 'GPTConfig', iterr: int = -10):
	...
	ix = torch.randint(len(data[0]), (config.batch_size, ))

	x = torch.stack([data[0][i] for i in ix])
	y = torch.stack([data[1][i] for i in ix])
	# x = torch.stack([data[0][i:i+config.block_size] for i in ix])
	# y = torch.stack([data[1][i+1:i+config.block_size+1] for i in ix])
...
# hyperparameters
class GPTConfig:
	def __init__(self, data: 'DataClass', vocab_size: int,
			  max_number: int, res_number: int, batch_size=64,
...
class DataClass:
	def __init__(self, X: torch.Tensor, Y: torch.Tensor, split: int = 0.9):
		n = int(split * len(X))
		self.train_data = [X[:n], Y[:n]]
		self.val_data = [X[n:], Y[n:]]
